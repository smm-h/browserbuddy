...
3. It posts the event to the background script over the extension's internal messaging.
...
- **JSONL log under `server/data/events/`.** One JSON object per line, appended as events arrive, into a file named for the **UTC day** (`YYYY-MM-DD.jsonl`). Rotation is by calendar day, not by server run: consecutive sessions on the same day share a file. Append-only, trivially greppable, and readable by anything that can read a line at a time. History beyond the ring lives here, and a failed append is fatal rather than silently dropped. It is read back **once**, at startup, to refill the ring (newest day file first, up to 1000 events); after that the MCP tools are served entirely from the ring.

### Sequence numbers

`seq` is assigned by the hub, densely and strictly increasing, across all events.
...
- **Persisted, and never reused.** The counter lives in `<dataDir>/seq.json` and resumes at persisted + 1 on startup; the ring is refilled from the JSONL log at the same time. The host is not long-lived — on the native carrier every background teardown respawns it while the MCP endpoint stays put — so a per-run counter would have quietly broken `sinceSeq` on a restart the client never saw. A corrupt or missing `seq.json` falls back to the highest `seq` in the log; it never resets to 1 while events exist.
...
- **Background script — tab-level, time-windowed.** When an RPC touches a tab, that tab is marked agent-driven for 1500 ms, and tab-level events for it are tagged `agent`. A time window is necessary here because the causal chain is asynchronous: `navigate` returns before the navigation commits, and the resulting `navigation` and `page_loaded` events arrive later through unrelated browser listeners. There is no token connecting them back to the RPC. 1.5 seconds comfortably covers a commit while staying short enough that a user action a moment afterwards is attributed correctly. Three effects cannot be keyed by tab id at all and get their own global windows: `newTab` (the browser fires `tabs.onCreated` before `tabs.create` resolves, so the id does not exist yet when the mark would have to be set), `download` (`downloads.onCreated` carries no tab), and window focus (`windows.onFocusChanged` carries no tab either, so `activateTab` opens a 1500 ms focus window immediately before focusing the window, and a `window_focus` inside it is reported as `agent`). `screenshot` marks the tab only in the case where it must activate a background tab to capture it — that activation is the agent's doing.
- **Background script — surviving a slow load.** A window measured in milliseconds cannot, by itself, cover a load measured in seconds, so two rules keep it honest. First, the actor is read **at listener time**: `tab_activated` and `page_loaded` must fetch the tab title before they can emit, and deciding the actor after that async hop would date the decision to whenever the fetch answered rather than to when the event happened. Second, a load inherits the actor of the commit that started it: `webNavigation.onCommitted` records its actor for the tab and `page_loaded` consumes that record instead of re-asking the clock, so a slow cross-origin load stays the agent's however long it takes. An agent commit also renews the tab's window, which carries the further commits of a redirect chain. The record is per tab and replaced by every commit, so the human's next navigation — recorded as `user` when it commits — takes its own completion with it.
...
- **`runJs` spans both.** It runs in the page's main world via the `scripting` API, so it cannot set the in-page flag from inside the content script. The background therefore does two things before injecting: it marks the tab (covering tab-level effects) and sends the content script an internal message that raises an in-page agent window of 1500 ms (covering DOM effects the injected code triggers). The window and the 100 ms RPC flag are independent deadlines; the later one wins. On a page where no content script can run, the message simply fails and `runJs` proceeds — there is nothing in-page to attribute there anyway.
...
## 8. MV3 background lifetime

Manifest V3 replaced the persistent background page with a terminable background context: a service worker on Chrome, an event page on Firefox. Both browsers kill it when idle — typically after about 30 seconds. Everything about the extension's connection management is shaped by this.

- **Top-level listener registration.** Every extension-API event listener is registered synchronously at the top level of `background.js`, never inside a callback, a promise, or an async initialiser. The browser uses the listener set to decide whether to wake a terminated worker for an event; a listener registered late may exist only when the worker happens to be alive, which produces events that are observed sometimes and missed other times. Registering at the top level is what makes observation reliable.
...
- **Alarms as a backstop.** An `alarms` timer firing every 30 seconds wakes the worker independently of the socket and re-attempts the connection, covering the case where the worker was terminated with the connection already down — a situation where neither a ping nor a pending retry timer can fire, because there is nothing running to fire them.
- **Event buffering with a `storage.session` mirror.** Events observed while the socket is down are queued, capped at 500 with the oldest dropped first, so a long outage loses the start of the gap rather than growing without bound. The queue lives in memory for speed and is mirrored to `storage.session`, which survives worker termination (and, unlike `storage.local`, is cleared when the browser closes — appropriate for transient events that may contain page content). Mirror writes are coalesced over 500 ms, except immediately after a successful full drain, which writes through at once: without that, a teardown inside the coalescing window would rehydrate a mirror still holding the events just sent and deliver them a second time. On restart the worker rehydrates the queue before connecting; on reconnect it drains in original order, and while anything is queued even live events join the back of the queue so nothing overtakes the backlog.
...
### Firefox: the keepalive does not keep alive

The mechanics above behave differently on the two browsers, and the difference was measured, not assumed. Chrome counts WebSocket traffic as service-worker activity, so the 20-second ping genuinely defers the idle teardown and an idle-but-connected extension stays connected. Firefox does not: WebSocket activity never resets the event page's idle timer, and neither do the alarms (an alarm wakes a suspended page but does not prevent the suspension). An idle Firefox extension therefore cycles — suspended after ~30 seconds without extension events, revived by the next 30-second alarm, reconnecting on wake — roughly one reconnect per minute, observed directly against a live hub. Shortening the alarm period was tested and only makes the cycle faster, so the shared 30-second constant stays.

The cycle is exactly the situation the architecture already treats as routine. Events observed while suspended wake the page, are buffered, and are delivered on the next reconnect, so the observed stream stays complete (delivery can lag by up to ~30 seconds at idle). During active browsing the user's own activity generates extension events that reset the idle timer, so the connection holds precisely when observation matters most. The one real consequence: an acting tool that lands in a suspended window fails with the not-connected error; a retry within ~30 seconds succeeds. That is a visible, retriable failure — consistent with §9 — not a degradation to be hidden.
...
- **The endpoint is discovered, not configured.** The first launch in a data directory picks an ephemeral port and generates a bearer token, and publishes both to `<data-dir>/mcp-endpoint.json` for the MCP client to read. This replaces the fixed 8590 and the compile-time hub URL in `background.js` — the reason §3 refuses to relocate the port does not apply when the *server* advertises itself.
- **…but it is stable once discovered.** Discovery alone would not be enough: the browser respawns the host on every background teardown, and a new port and token each time would silently break the client that had already been configured — the "no silent degradation" rule in reverse. So the host persists its identity in `<data-dir>/endpoint-state.json` (mode `0600`) and, on the next launch, reuses the token unconditionally and rebinds the same port. `SO_REUSEADDR` makes the previous host's `TIME_WAIT` sockets irrelevant, so a respawn is normally invisible to the client: same url, same token. Only a port taken by a *live* listener forces an ephemeral rebind, and that is announced loudly on stderr rather than absorbed. This is what makes the carrier's "the browser owns the host's lifetime" property survivable for a long-lived MCP session.
- **A dead host is not an endpoint.** The descriptor records the host's pid, and the reader checks it: a file left behind by a killed host reads as "no endpoint", so nothing dials a port that may since belong to an unrelated process.
- **Discovery needs a front door, and it is a CLI command.** A published descriptor is only half an answer: something has to turn it into a client registration, and asking the user to hand-copy a bearer token out of a JSON file is how setup instructions go stale. `browserbuddy client-config` reads the descriptor and prints the exact `claude mcp add --transport http … --header "Authorization: Bearer …"` line and the equivalent `mcpServers` block; `--apply` executes it. It is print-by-default on purpose — the inverted topology already surprises people, and a command that answered "where is the endpoint?" by silently editing their MCP client's configuration would compound that. Its failure mode carries the entire setup order (install the host → load the extension → ask again), because the commonest reason for no endpoint is not a crash but a browser that has not spawned the host yet, and that is not something retrying can fix. This is also the CLI's only stdout writer: a registration is a *result*, it belongs on fd 1 where it can be piped, and a test keeps `ctx.info` confined to that one file so `serve`'s fd 1 stays the MCP stdio protocol's alone.
- **Result size is a per-direction property.** The browser caps host → browser messages at 1 MB and allows up to 4 GB the other way, which is exactly the asymmetry the traffic has: commands out are tiny, results in (an evaluated value, a screenshot) are not. Bounding the inbound decoder at the outbound limit was a real defect — a single large result closed the channel, which killed the host and its HTTP endpoint. Inbound is bounded generously (128 MB, corruption detection only); each result is bounded at the source instead (64 MB in the extension), where the failure can be one `ok:false` RPC rather than a lost session.
...
The native port behaves like the WebSocket did on Chrome: traffic on it resets the service worker's idle timer, so the 20-second `ping` keeps the worker — and therefore the host — alive across long idle periods. Measured directly against Chromium 145 with a bare probe extension, a port with one message every 20 seconds and no other activity stayed up for the full length of the run without a single disconnect.

**On Firefox the native carrier does not reproduce the WebSocket carrier's idle cycling, and that was measured too.** §8 records that Firefox never counts WebSocket traffic as event-page activity, so an idle extension on that carrier suspends after ~30 seconds and cycles roughly once a minute. A connected native port is different: `node scripts/spike-nativemsg.mjs --browser firefox --idle-probe-sec 180` against Firefox 148 sat idle for three minutes with only the 20-second `ping` on the pipe, and afterwards the tools answered immediately *and the endpoint descriptor still named the same host pid* — the event page was never suspended, because a suspension would have closed the pipe and killed that process. The pid, not the tool call, is the evidence: the endpoint identity is designed to make a respawn invisible to the client, so "a tool still works" would have proved nothing on its own.

That makes the Firefox limitation carrier-specific rather than browser-specific. It is not a reason to relax anything: the reconnect ladder, the event buffer and the persisted endpoint identity all still apply, because a teardown is a matter of *when*, not *whether* — the browser can drop the background context for reasons that have nothing to do with idleness.

A teardown is heavier here than on the WebSocket carrier: it does not merely drop a socket that can be redialled, it kills the host process and, with it, the HTTP endpoint. The persisted endpoint identity above is what makes that survivable — the respawned host comes back on the same port with the same token, so the client's configured URL resumes working instead of going permanently dead. The keepalive still matters (an endpoint that never goes away beats one that comes back), but it is no longer the only thing standing between a routine worker teardown and a broken MCP session.
...
| Screenshots capture only the visible tab | The target tab is activated first, changing the user's foreground tab. | Neither browser offers off-screen capture to extensions. Activating first is at least predictable and visible, rather than failing on any non-active tab. |
| Screenshots on Firefox require a user gesture | `screenshot` fails on Firefox until the user clicks the BrowserBuddy toolbar button on the tab (granting `activeTab`, which lasts until the tab navigates). | Firefox MV3 never treats granted host permissions as the capture permission (`hasPermission("<all_urls>")` ignores origin-controls grants), so gesture-less capture is impossible. The error names the workaround; a silent blank image would be worse. |
| Firefox suspends the background at idle | At idle, the extension reconnects roughly once a minute; an acting tool issued in a suspended window fails with the not-connected error and succeeds on retry. | Firefox does not count WebSocket traffic as background activity (see §8). Events are buffered across the cycle, so observation stays lossless; the transient failure is visible and retriable rather than hidden. |
| One extension connection at a time | A second browser profile cannot connect concurrently; the newer connection displaces the older. Chrome and Firefox cannot be connected at once. | Multiple connections would require routing every RPC by browser identity and partitioning the event stream. Single-profile is the actual use case; the newest-wins rule makes extension reloads resolve deterministically. |
| No browser-internal pages | `chrome://` and `about:` pages, the Chrome Web Store, addons.mozilla.org and other extensions are invisible and untouchable. | A browser restriction, not a design choice. Nothing to work around. |
| The event log is re-read at startup | Booting a host with a large day file costs one file read before it serves. | The endpoint outlives any single host process, so a cursor must too. Reading newest-first and stopping at 1000 bounds the cost to roughly one ring's worth, and it buys "everything since I last looked" across a restart the client never observed. |
| Attribution window is time-based | A user action within ~1.5 s of an agent command in the same tab may be labelled `agent`. | The browsers' navigation events carry no correlation token back to the initiating call. The error is conservative — the assistant under-reacts rather than mistaking its own action for the user's. |
