...
- Framing on that pipe is the browser's, not ours: each message is a 32-bit **little-endian** byte length followed by exactly that many bytes of UTF-8 JSON.
- **The two directions have different size limits, and the difference is normative.** Host → browser is capped at **1 MB** by the browser, so the host enforces a slightly smaller cap on its writer: an outgoing frame above it is a hard error naming the `rpc` method it was carrying, never a truncation. Browser → host may be up to **4 GB**, which is the direction results travel; the host's decoder therefore accepts up to **128 MB** and treats a larger length header as a corrupt stream. Applying the outbound cap inbound would turn one large `runJs` or `screenshot` result into a closed channel and a dead host.
- **The extension bounds each result before it sends it.** A `runJs` value or a `screenshot` base64 blob above **64 MB** is answered as `{"ok":false}` naming the measured size and the limit (`result too large (N bytes, limit 64MB)`). One call fails; the pipe is never torn down to report a size.
...
- The host serves the MCP surface over Streamable HTTP on `127.0.0.1`, path `/mcp`, protected by a bearer token. Requests without a matching `Authorization: Bearer …` header get `401`; no CORS headers are emitted, so a web page cannot reach the endpoint.
- The host writes `<data-dir>/mcp-endpoint.json` (mode `0600`, written to a temp file and renamed, so a reader never sees a partial file) with `url`, `token`, `pid` and a ready-to-paste `mcpServers` block. It removes the file on shutdown. **A file whose `pid` is no longer alive is stale, and this is enforced, not merely declared**: the reader checks the recorded pid with `kill(pid, 0)` and reports a stale file as no endpoint at all. A descriptor without an integer `pid` is a hard error — it is not a state the host ever writes.

#### Endpoint stability across host restarts

The browser tears its background context down routinely, and every teardown closes the pipe, kills the host, and respawns it on the next connect. A fresh port and token per launch would silently invalidate the URL an MCP client was configured with — a client that worked a minute ago would start getting connection refused, or `401`. So the endpoint has an identity that outlives one launch:

- The host keeps `<data-dir>/endpoint-state.json` (mode `0600`) holding the `token` and `port` of the previous launch. A malformed one is a hard error; delete it to mint a fresh identity.
- On startup it **reuses the recorded token unconditionally** and **attempts to bind the recorded port**. `SO_REUSEADDR` (Node's default) means the previous host's `TIME_WAIT` sockets do not stand in the way, so the normal outcome is the same url and the same token as before, and the client notices nothing.
- If that port is held by a live listener, the host binds an **ephemeral** port instead, reports the reason on stderr, and rewrites the descriptor. The url changes; the token still does not.
- Both files are rewritten with the port actually bound: `endpoint-state.json` for the next respawn, `mcp-endpoint.json` for the client. The first launch in a fresh data directory generates a token and binds an ephemeral port.
- **The descriptor is the whole client-facing contract, and one command reads it.** `browserbuddy client-config` resolves the descriptor with the same liveness rule, and prints the registration a client needs: `claude mcp add --transport http browserbuddy <url> --header "Authorization: Bearer <token>"`, plus the equivalent `mcpServers` block. With no live descriptor it fails and names the ordered procedure — install the host, load the extension (which spawns it), then ask again — because a client cannot be configured before the browser has started the server. `--apply` runs the registration instead of printing it; printing is the default, since reading the endpoint must not mutate a client's configuration. Any other MCP client reads the same file: `url` and `token`, or the `mcpServers` block verbatim.
- Host manifest locations: `<user-data-dir>/NativeMessagingHosts/com.browserbuddy.host.json` for Chrome/Chromium (it follows `--user-data-dir`, so a test profile can install its own), and `$HOME/.mozilla/native-messaging-hosts/com.browserbuddy.host.json` for Firefox (keyed on `HOME`, not on the profile). The Chrome manifest names the extension in `allowed_origins`, which requires a stable extension id — hence the `key` pinned in `extension/manifest.json`. The Firefox manifest names the gecko id in `allowed_extensions`. `browserbuddy install-host --browser chrome|firefox` writes both the manifest and the launcher its `path` points at; the install is Linux-only in this version and refuses other platforms outright rather than writing where no browser looks.
- A failed spawn and a later disconnect are different conditions and are reported differently. `connectNative` throwing, or a port that disconnects before the host has sent a single message, is a **spawn failure**: the extension reports the full install instructions and keeps an error badge up. A pipe that closes after the host has spoken is a routine disconnect (browser shutdown, background teardown, host crash): a neutral state, and the reconnect ladder of §1.2 applies.
...
- `screenshot` and `runJs` are the two methods whose result can be large. Both are measured before they go on the wire and fail with `result too large (N bytes, limit 64MB)` above the per-result limit of §1.1. A result under that limit is delivered whole, even far above 1 MB.
